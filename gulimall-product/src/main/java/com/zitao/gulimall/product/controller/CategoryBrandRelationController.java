...
import com.zitao.gulimall.product.vo.BrandVo;
...
public class CategoryBrandRelationController {
    ...
    public R relationBrandsList(@RequestParam(value = "catId", required = true) Long catelogId) {
        List<BrandEntity> vos = categoryBrandRelationService.getBrandsByCatId(catelogId);
        List<BrandVo> collect = vos.stream().map(item -> {
            BrandVo brandVo = new BrandVo();
            brandVo.setBrandId(item.getBrandId());
            brandVo.setBandName(item.getName());
            return brandVo;
